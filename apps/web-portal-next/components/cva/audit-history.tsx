...
export function AuditHistory() {
    ...
    listVerificationRequests({ size: 200 })
      ...
      .catch((err) => {
        setState((prev) => ({
          ...prev,
          isLoading: false,
          error: err.message ?? "Failed to load audit history",
        }));
      });
  }, []);
  ...
  const handleDownloadPdf = async (request: VerificationRequest) => {
    ...
    try {
      ...
      toast({
        title: "Report ready",
        description: "Carbon audit PDF downloaded.",
      });
    } catch (err) {
      const message =
              ...
              >
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted-foreground">
                    ...
                    {request.verifiedAt && (
                      <span>Reviewed {formatDateTime(request.verifiedAt)}</span>
                    )}
                    {request.creditIssuance && (
                      <span>
                        Credits{" "}
                        {Number(request.creditIssuance.creditsRounded).toFixed(
                          2
                        )}{" "}
                        (CO₂{" "}
                        {Number(request.creditIssuance.co2ReducedKg).toFixed(2)}{" "}
                        kg)
                      </span>
                    )}
                  </div>
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="max-w-xl">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Carbon audit metadata for request{" "}
                      ...
                      <div className="space-y-4">
                        <Section title="Status">
                          ...
                          {request.verifierId && (
                            <p className="text-sm text-muted-foreground">
                              Verifier {truncateId(request.verifierId)}
                            </p>
                          )}
                        </Section>
                        ...
                        <Section title="Metrics">
                          <DetailRow label="Distance (km)">
                            {request.distanceKm.toFixed(2)}
                          </DetailRow>
                          <DetailRow label="Energy (kWh)">
                            {request.energyKwh.toFixed(2)}
                          </DetailRow>
                        </Section>
                        {request.creditIssuance && (
                          <Section title="Credit issuance">
                            <DetailRow label="Issuance ID">
                              {request.creditIssuance.id}
                            </DetailRow>
                            <DetailRow label="Issued at">
                              {formatDateTime(request.creditIssuance.createdAt)}
                            </DetailRow>
                            <DetailRow label="Credits rounded">
                              {Number(
                                request.creditIssuance.creditsRounded
                              ).toFixed(2)}
                            </DetailRow>
                            <DetailRow label="Credits raw">
                              {Number(
                                request.creditIssuance.creditsRaw
                              ).toFixed(4)}
                            </DetailRow>
                            <DetailRow label="CO₂ reduced (kg)">
                              {Number(
                                request.creditIssuance.co2ReducedKg
                              ).toFixed(3)}
                            </DetailRow>
                            <DetailRow label="Idempotency key">
                              {request.creditIssuance.idempotencyKey}
                            </DetailRow>
...
function truncateId(value: string | number | undefined | null, length = 6) {
  ...
  // Chuyển sang chuỗi trước khi xử lý
